namespace :cute_admin do
  ...
  task :generate_with_ajax => :environment do
    ...
    for pmodel in process_models do
      require 'rails_generator'
      require 'rails_generator/scripts/generate'
      Rails::Generator::Scripts::Generate.new.run(["cute_admin", pmodel, "--use-ajax"])
      puts "Created cute_admin for #{pmodel.camelize}"
    end
  end
  ...
  task :generate_with_associations => :environment do
    ...
    for pmodel in process_models do
      require 'rails_generator'
      require 'rails_generator/scripts/generate'
      Rails::Generator::Scripts::Generate.new.run(["cute_admin", pmodel, "--add-associated"])
      puts "Created cute_admin with associations for #{pmodel.camelize}"
    end
  end
  ...
  task :generate_with_ajax_and_associations => :environment do
    ...
    for pmodel in process_models do
      require 'rails_generator'
      require 'rails_generator/scripts/generate'
      Rails::Generator::Scripts::Generate.new.run(["cute_admin", pmodel, "--add-associated", "--use-ajax"])
      puts "Created cute_admin with associations for #{pmodel.camelize}"
    end
  end
